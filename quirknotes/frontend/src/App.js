...
function App() {
  ...
  const deleteNote = async (entry) => {
    ...
    try {
      await fetch(`http://localhost:4000/deleteNote/` + entry._id.toString(), {
        method: "DELETE"
      })
      .then(async (response) => {
        if (!response.ok) {
  ...
  const deleteNoteState = (entry) => {
    ...
    let id = entry.slice(17,41) 
    setNotes((notes) => notes = notes.filter((entry) => entry._id.toString() !== id))
  }
